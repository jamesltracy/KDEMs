...
##########################################################################################
### Loop through three training KDE surfaces to normalize them from zero to one and
### create KDE models (KDEMs)for each year. Alsom extract presence,
### and pseudoabsence data for model calibration to maximize the true skill statistic
...
# Year="02_16"
  for(BigLoop in BigLoopCount) {
    ...
    setwd(OutDirectpsa)
    output3 <- paste0("KDEMProj_", Year, "_", SetNameF, "_ThresholdCalib", BigLoop, NObskfoldgrppL[BigLoop])
